<?php
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        //$nome = $_POST["nome"]; SEM PROTEÇÂO
        $nome = htmlspecialchars($_POST["nome"], ENT_QUOTES, 'UTF-8'); //COM PROTEÇÂO
        $email = filter_input(INPUT_POST, "email", FILTER_VALIDATE_EMAIL); //COM PROTEÇÂO
        //echo "<p> Valores recebidos: nome $nome - email: $email<p>";
        $idade = htmlspecialchars($_POST["idade"]); //COM PROTEÇÂO
        $curso = htmlspecialchars($_POST["curso"]); //COM PROTEÇÂO
        if(empty($nome)){
